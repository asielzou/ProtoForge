  ...
  <n-layout v-else has-sider class="app-layout">
    ...
    >
      <div class="sider-logo" @click="$router.push('/')">
        <svg v-if="!collapsed" viewBox="0 0 24 24" width="28" height="28" fill="none" stroke="url(#logoGrad)" stroke-width="2">
        ...
        <svg v-else viewBox="0 0 24 24" width="28" height="28" fill="none" stroke="#6366f1" stroke-width="2">
          <path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5"/>
        </svg>
      </div>
      <n-menu
...
function onSearchInput(query) {
  ...
  for (const t of searchData.value.templates) {
    if ((t.name || '').toLowerCase().includes(q))
      results.push({ label: `[模板] ${t.name}`, value: `/marketplace` })
  }
